...
Comprehensive verification across PVT corners using TSMC7 naive modelcards.
...
Duration: ~10 minutes
Requires: NGSPICE, TSMC7 naive modelcards
"""
...
# TSMC7 modelcard configuration — use naive (pre-baked) modelcards
...
# TSMC7 naive modelcard mapping: key -> (modelcard_file, model_name)
...
TSMC7_PMOS_LVT = {
    ...
    # PDIBL2_i=-0.118 from binning params. L=20nm works correctly.
...
def _get_tsmc7_naive_modelcard(device_spec: dict) -> Path:
    """Get path to a TSMC7 naive modelcard file."""
    ...
    card = TSMC7_NAIVE_DIR / device_spec["file"]
    if not card.exists():
        raise FileNotFoundError(f"TSMC7 naive modelcard not found: {card}")
    return card
...
def test_tsmc7_nmos_svt_op() -> None:
    ...
    """
    spec = TSMC7_NMOS_SVT
    modelcard = _get_tsmc7_naive_modelcard(spec)
...
@pytest.mark.fast
@pytest.mark.skipif(not OSDI_PATH.exists(), reason="missing OSDI build artifact")
def test_tsmc7_pmos_lvt_op() -> None:
    """Verify TSMC7 PMOS LVT at nominal operating point.

    Tests a single operating point for PMOS LVT device (pch_lvt_mac) at L=16nm.
    ...
    # PMOS: Vd=0, Vg=0, Vs=0.75 (Vgs = 0 - 0.75 = -0.75V, PMOS ON)
    ng_result = _run_tsmc7_ngspice_op_point(
        modelcard=modelcard,
        model_name=model_name,
        inst_params=inst_params,
