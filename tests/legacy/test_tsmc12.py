"""
TSMC12 PVT Verification Tests

Comprehensive verification across PVT corners using TSMC12 naive modelcards.
...
Run: pytest tests/test_tsmc12.py -v
Duration: ~10 minutes
Requires: NGSPICE, TSMC12 naive modelcards
"""
...
# TSMC12 modelcard configuration — use naive (pre-baked) modelcards
TSMC12_NAIVE_DIR = ROOT / "tech_model_cards" / "TSMC12" / "naive"
TSMC12_MODELCARD_OVERRIDE = os.environ.get("TSMC12_MODELCARD")

NGSPICE_BIN = os.environ.get("NGSPICE_BIN",
                              "/usr/local/ngspice-45.2/bin/ngspice")

# TSMC12 naive modelcard mapping: key -> (modelcard_file, model_name)
# Naive modelcards have ONE model per file with all params baked in.
# Note: pch_svt_mac_l16nm.l does NOT exist — use pch_lvt_mac at 20nm instead.
TSMC12_NMOS_SVT = {
...
TSMC12_TEST_TEMPS = [-40, 27, 85, 125]

# Representative voltage points for TSMC12 (0.80V core)
TSMC12_VG_CORE = [0.0, 0.3, 0.6, 0.8, 1.0]
TSMC12_VD_CORE = [0.0, 0.3, 0.6, 0.8]


def _get_tsmc12_naive_modelcard(device_spec: dict) -> Path:
    """Get path to a TSMC12 naive modelcard file."""
    if TSMC12_MODELCARD_OVERRIDE:
        override = Path(TSMC12_MODELCARD_OVERRIDE)
        if override.is_file():
            return override
        raise FileNotFoundError(f"TSMC12 override not found: {override}")

    card = TSMC12_NAIVE_DIR / device_spec["file"]
    if not card.exists():
        raise FileNotFoundError(f"TSMC12 naive modelcard not found: {card}")
    return card
...
def _run_tsmc12_ngspice_op_point(modelcard: Path, model_name: str,
                                  inst_params: dict,
                                  vd: float, vg: float,
                                  vs: float, ve: float, temp_c: float) -> dict:
    """Run NGSPICE operating point analysis for TSMC12.
    ...
    """
    out_dir = BUILD / "ngspice_eval" / "tsmc12"
...
def test_tsmc12_nmos_svt_op() -> None:
    """Verify TSMC12 NMOS SVT at nominal operating point.

    Quick smoke test: single OP comparison for NMOS SVT (nch_svt_mac) at L=16nm.
    Uses naive modelcard with all params pre-baked.
    """
    spec = TSMC12_NMOS_SVT
    modelcard = _get_tsmc12_naive_modelcard(spec)
    model_name = spec["model"]
    inst_params = spec["params"]

    # Test at nominal operating point (Vdd = 0.80V for TSMC12)
    ng_result = _run_tsmc12_ngspice_op_point(
        modelcard=modelcard,
...
@pytest.mark.fast
@pytest.mark.skipif(not OSDI_PATH.exists(), reason="missing OSDI build artifact")
def test_tsmc12_pmos_lvt_op() -> None:
    """Verify TSMC12 PMOS LVT at nominal operating point.
...
@pytest.mark.slow
@pytest.mark.skipif(not OSDI_PATH.exists(), reason="missing OSDI build artifact")
def test_tsmc12_temperature_sweep() -> None:
    """Test TSMC12 temperature sweep at representative temperatures.
    ...
    for temp_c in TSMC12_TEST_TEMPS:
        ng_result = _run_tsmc12_ngspice_op_point(
            modelcard=modelcard,
            model_name=model_name,
            inst_params=inst_params,
...
@pytest.mark.slow
@pytest.mark.skipif(not OSDI_PATH.exists(), reason="missing OSDI build artifact")
def test_tsmc12_voltage_sweep() -> None:
    """Test TSMC12 Id-Vg and Id-Vd sweeps at representative voltage points.

    Verifies model accuracy across the operating voltage range for TSMC12 (0 to 1.0V)
    using NMOS SVT naive modelcard.
    """
    spec = TSMC12_NMOS_SVT
    modelcard = _get_tsmc12_naive_modelcard(spec)
