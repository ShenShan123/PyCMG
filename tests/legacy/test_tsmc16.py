"""
TSMC16 PVT Verification Tests

Comprehensive verification across PVT corners using TSMC16 naive modelcards.
...
Run: pytest tests/test_tsmc16.py -v
Duration: ~10 minutes
Requires: NGSPICE, TSMC16 naive modelcards
"""
...
# TSMC16 modelcard configuration -- use naive (pre-baked) modelcards
# IMPORTANT: TSMC16 has BOTH RF and non-RF models. Use ONLY non-RF models
# (nch_svt_mac, NOT nch_svt_mac_rf)
TSMC16_NAIVE_DIR = ROOT / "tech_model_cards" / "TSMC16" / "naive"
TSMC16_MODELCARD_OVERRIDE = os.environ.get("TSMC16_MODELCARD")

NGSPICE_BIN = os.environ.get("NGSPICE_BIN",
                              "/usr/local/ngspice-45.2/bin/ngspice")

# TSMC16 naive modelcard mapping: key -> (modelcard_file, model_name)
# Naive modelcards have ONE model per file with all params baked in.
# Note: pch_svt_mac_l16nm.l does NOT exist -- use pch_lvt_mac at 16nm instead.
TSMC16_NMOS_SVT = {
...
TSMC16_PMOS_LVT = {
    # Note: L=16nm PMOS causes NGSPICE convergence failure due to invalid
    # PDIBL2_i from binning params. L=20nm works correctly.
...
TSMC16_TEST_TEMPS = [-40, 27, 85, 125]

# Representative voltage points for TSMC16 (0.80V core)
TSMC16_VG_CORE = [0.0, 0.3, 0.6, 0.8, 1.0]
TSMC16_VD_CORE = [0.0, 0.3, 0.6, 0.8]


def _get_tsmc16_naive_modelcard(device_spec: dict) -> Path:
    """Get path to a TSMC16 naive modelcard file."""
    if TSMC16_MODELCARD_OVERRIDE:
        override = Path(TSMC16_MODELCARD_OVERRIDE)
        if override.is_file():
            return override
        raise FileNotFoundError(f"TSMC16 override not found: {override}")

    card = TSMC16_NAIVE_DIR / device_spec["file"]
    if not card.exists():
        raise FileNotFoundError(f"TSMC16 naive modelcard not found: {card}")
    return card
...
def _run_tsmc16_ngspice_op_point(modelcard: Path, model_name: str,
                                  inst_params: dict,
                                  vd: float, vg: float,
                                  vs: float, ve: float, temp_c: float) -> dict:
    """Run NGSPICE operating point analysis for TSMC16.
    ...
    """
    out_dir = BUILD / "ngspice_eval" / "tsmc16"
...
def test_tsmc16_nmos_svt_op() -> None:
    """Verify TSMC16 NMOS SVT at nominal operating point.

    Quick smoke test: single OP comparison for NMOS SVT (nch_svt_mac) at L=16nm.
    Uses naive modelcard with all params pre-baked.
    """
    spec = TSMC16_NMOS_SVT
    modelcard = _get_tsmc16_naive_modelcard(spec)
    model_name = spec["model"]
    inst_params = spec["params"]

    # Test at nominal operating point (Vdd = 0.80V for TSMC16)
    ng_result = _run_tsmc16_ngspice_op_point(
        modelcard=modelcard,
        model_name=model_name,
        inst_params=inst_params,
        vd=0.80, vg=0.80, vs=0.0, ve=0.0,
        temp_c=27.0
    )
    ...
    py_result = inst.eval_dc({"d": 0.80, "g": 0.80, "s": 0.0, "e": 0.0})
    ...
    print(f"NMOS SVT: Id={py_result['id']:.3e}A gm={py_result['gm']:.3e}S")


@pytest.mark.fast
@pytest.mark.skipif(not OSDI_PATH.exists(), reason="missing OSDI build artifact")
def test_tsmc16_pmos_lvt_op() -> None:
    """Verify TSMC16 PMOS LVT at nominal operating point.

    Tests a single operating point for PMOS LVT device (pch_lvt_mac) at L=20nm.
    Note: pch_svt_mac_l16nm.l does not exist, so we use pch_lvt_mac instead.
    """
    spec = TSMC16_PMOS_LVT
    modelcard = _get_tsmc16_naive_modelcard(spec)
    model_name = spec["model"]
    inst_params = spec["params"]

    # PMOS: Vd=0, Vg=0, Vs=0.80 (Vgs = 0 - 0.80 = -0.80V, PMOS ON)
    ng_result = _run_tsmc16_ngspice_op_point(
        modelcard=modelcard,
        model_name=model_name,
        inst_params=inst_params,
        vd=0.0, vg=0.0, vs=0.80, ve=0.0,
        temp_c=27.0
    )

    model = Model(str(OSDI_PATH), str(modelcard), model_name)
    inst = Instance(model, params=inst_params)
    py_result = inst.eval_dc({"d": 0.0, "g": 0.0, "s": 0.80, "e": 0.0})
    ...
    print(f"PMOS LVT: Id={py_result['id']:.3e}A Ig={py_result['ig']:.3e}A")


@pytest.mark.slow
@pytest.mark.skipif(not OSDI_PATH.exists(), reason="missing OSDI build artifact")
def test_tsmc16_temperature_sweep() -> None:
    """Test TSMC16 temperature sweep at representative temperatures.
    ...
    for temp_c in TSMC16_TEST_TEMPS:
        ng_result = _run_tsmc16_ngspice_op_point(
            modelcard=modelcard,
            model_name=model_name,
            inst_params=inst_params,
...
@pytest.mark.slow
@pytest.mark.skipif(not OSDI_PATH.exists(), reason="missing OSDI build artifact")
def test_tsmc16_voltage_sweep() -> None:
    """Test TSMC16 Id-Vg and Id-Vd sweeps at representative voltage points.

    Verifies model accuracy across the operating voltage range for TSMC16 (0 to 1.0V)
    using NMOS SVT naive modelcard.
    """
    spec = TSMC16_NMOS_SVT
    modelcard = _get_tsmc16_naive_modelcard(spec)
